...
| petekIO      | 0.3.4   | `pip install petekio`      | live                |
| petekStatic  | 0.1.7   | `pip install petekstatic`  | live                |
| petekSim     | 0.1.5   | `pip install peteksim`     | live                |

### Rust (crates.io)

```sh
cargo add petektools@0.2.4     # TOOLKIT — numeric kernels, units, container
cargo add petekio@0.3.4        # DATA    — ingest + model-ready inputs
cargo add petekstatic@0.1.7    # GEOMODEL — StaticModel build + volumetrics + MC
cargo add peteksim@0.1.5       # SIMULATION — the appraisal facade over the stack
```
...
pip install petekio==0.3.4
pip install petekstatic==0.1.7   # static workflow API, StaticModel, volumes + bundles
pip install peteksim==0.1.5      # the full appraisal facade over the whole stack
```
